...
private void selectCard()
 ...
 Debug.Log("Index: " + new_index + " name: " + hitPoint.transform.GetComponentInParent<Hexagon>().transform.name);                    //hex.material = highlightedHex;
                    if (new_index != hoveredHex)
                    {
                        if (phase == 1)
                        {
                            Debug.Log("phase 1 select single");                            selectSingle(new_index);
                        }
